...
哈啰:
java多态的底层原理
system.arraycopy底层原理

outofmemory是怎么造成的，实际中怎么处理
线程池参数，丢弃策略有哪些，实际是怎么处理
...
为什么不航空报国 做程序员
学习的课程
MySQL插入7条数据，删除2条数据，重启后，在插入一条数据，生成的自增主键id是啥
mysql int(11), 11是什么，范围
MySQL出现问题怎么排除
执行计划看哪些字段
覆盖索引
tcp粘包
mybatis分页
springaop
jvm垃圾回收器,cms是标记清除还是整理

登录 原理 cookie,set-cookie
文本 敏感词列表 判断是否含有
kill -9杀死进程的原理
...
redis 缓存一致性
cache aside 问题，如何避免，设置缓存时间、延时双删
缓存击穿，锁、预热
...
斗鱼
object方法
int最大值，Integer.MAX_VALUE + 1 =
arraylist 和linkedlist 底层
100万数据 get(i) i 从0 到10**6 哪个快，怎么调整速度一致
hashmap 源码，put流程
concurrenthashmap 用到哪些锁

并发，synchronized
reentrantlock
aqs原理，公平锁和非公平锁

mysql 存储引擎，myisam和innodb引擎的区别
select count(*) 哪个快
两次select count(*)中间另一个事务delete一条数据，两次查询结果一致吗？为什么
事务隔离级别
mvcc字段多出来的字段
二级索引查询流程
mysql 主从复制涉及哪些线程
...
bean的作用域，springmvc默认的作用域
bean的生命周期
依赖注入的方式，问题
autowired,对象不存在时能正常启动吗

200万mysql数据，怎么维持redis中都是热点数据

dubbo的模型
消费者是怎么发现服务的

抽奖活动，20万数据等概率地抽取100名用户

斗鱼点击按钮，到服务器处理经过了哪些组件
系统架构，高可用高并发
ip转发，是直接访问到服务器吗
20亿数据，怎么分库分表，依据
...
java 线程状态
wait() synchronized线程消耗cpu时间吗
怎么查看java线程信息，jps看哪些参数
synchronized 关键字
synchronized 偏向锁，轻量级锁、自旋锁到重量级锁的升级
不加锁保护共享资源
cas，修改变量失败怎么办
java 哪些地方用了cas
aba问题，怎么解决
...
innodb默认事务隔离级别
为什么说可重复读部分解决了幻读问题，怎么解决的，哪里没解决
可重复读和读已提交都依赖于幻读，怎么实现的
不可重复读和幻读区别
update where 走索引，什么时候表锁，间隙锁、行锁
主键索引和二级索引的区别，主键索引只有一个吗
怎么创建索引
索引创建越多越好吗
怎么参看走索引的情况，explain执行计划看哪些字段

redis数据结构，hashmap的底层实现

登录的实现，app登录实现，token和cookie的区别，拦截器app没有cookie
session和token登录实现，session和cookie区别，怎么定位到session的
websocket位于网络的哪一层


线程池 核心参数 工作流程，核心线程数数量，实际项目中怎么用线程池

mysql 有哪些索引
MySQl 有哪些锁
意向锁是做怎么的
只有id= 1 和id=5 两条数据
select id = 4 from by update， 在聚簇索引和普通二级索引的执行加锁情况
update 语句在存储引擎层执行过程，涉及哪些日志
buffer pool什么时候刷新磁盘

redis 看过哪些书籍，使用过哪些数据结构
redis分布式锁，实际中过期时间怎么设置，现有的解决方案

token 被劫持，现有方案怎么解决
io 模型，了解哪些网络通信框架

dubbo 是怎么工作的，序列化
观察者模式，装饰器模式，java 哪些地方有用到装饰器模式
spring 用了哪些设计模式
spring aop实现 jdk动态代理涉及的两个类
...
浙商银行
spring bean的作用域
bean的创建过程，spring事务的传播级别

mysql 的事务隔离级别解决了什么问题
设计模式，双重检测锁
java 并发，怎么设计的线程池

redis的数据类型，除了做缓存还可以做什么
...
怎么记录日志
filter inteptor aop的执行先后顺序
spring 请求服务流程，dispatcher
mybatis 是怎么为dao接口生产代理类的
mybatis 怎么写sql 语句